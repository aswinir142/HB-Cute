...
async def delete_reaction_name(client, message: Message):
    ...
    if not ok:
        return await message.reply_text(
            "⚠️ Only admins or sudo users may remove triggers.\n"
            f"Debug: {debug}"
        )
...
async def list_reactions(client, message: Message):
    if not custom_mentions:
        return await message.reply_text("ℹ️ No reaction triggers found.")
...
async def clear_reactions(client, message: Message):
    ...
    if not ok:
        return await message.reply_text(
            "⚠️ Only admins or sudo can clear.\n"
            f"Debug: {debug}"
        )

    await COLLECTION.delete_many({})
    custom_mentions.clear()
    await message.reply_text("🧹 All triggers cleared.")

...
__HELP__ = """
Federation (multi-chat ban) management.
Commands:
/newfed <name> - create a federation (private chat only)
/delfed <fed_id> - delete a federation (private chat only)
/joinfed <fed_id> - join this group to a federation (group owner only)
/leavefed - leave current federation (group owner only)
/myfeds - list federations you created (private chat)
/renamefed <fed_id> <newname> - rename a federation (owner only)
/fedchats <fed_id> - list chats in the federation (owner/fadmins)
/fedinfo <fed_id> - info about a federation
/fpromote <user> - promote to fed-admin (owner or sudo)
/fdemote <user> - demote fed-admin
/fban <user> <reason> - federational ban
/unfban <user> <reason> - federational unban
/fbroadcast (reply to message) - broadcast message to all fed chats
"""

SUPPORT_CHAT = "https://t.me/HeartBeat_Fam"  # fallback


# Commands: using default "/" handler -- compatible across forks
...
async def new_fed(client, message):
    ...
    if not fed_name:
        return await message.reply_text("Please write down the name of the federation")
    fed_id = str(uuid.uuid4())
    x = await client.db["federation"].update_one(  # if your project uses db alias on client
        {"fed_id": fed_id},
        {
            "$set": {
                "fed_name": fed_name,
                ...
                "fadmins": [],
                "owner_mention": message.from_user.mention,
                "banned_users": [],
                "chat_ids": [],
    ...
    try:
        await app.send_message(LOG_GROUP_ID, f"New Federation: <b>{fed_name}</b>\nID: <pre>{fed_id}</pre>", parse_mode=ParseMode.HTML)
    except Exception:
        # logging fallback — do not crash
        pass
...
async def del_fed(client, message):
    ...
    if len(args) <= 1:
        return await message.reply_text("What should I delete? Provide a Fed ID.")
    fed_id = args[1].strip()
    getinfo = await get_fed_info(fed_id)
    if getinfo is False:
        return await message.reply_text("This federation does not exist.")
    # permission check: only fed owner or global sudo/owner
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("⚠️ Delete Federation ⚠️", callback_data=f"rmfed_{fed_id}")],
...
async def fedtransfer(client, message):
    ...
    is_feds = await get_feds_by_owner(int(message.from_user.id))
    if not is_feds:
        return await message.reply_text("**You haven't created any federations.**")
    if len(message.command) < 2:
        return await message.reply_text("**You need to specify a user or reply to their message!**")
    user_id, fed_id = await extract_user_and_reason(message)
    if not user_id:
        return await message.reply_text("I can't find that user.")
    if not fed_id:
        return await message.reply_text("You need to provide a Fed Id. Usage:\n/fedtransfer @username Fed_Id.")
    if not (await is_user_fed_owner(fed_id, message.from_user.id)):
        return await message.reply_text("Only federation owners can do this!")
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("⚠️ Transfer Federation ⚠️", callback_data=f"trfed_{user_id}|{fed_id}")],
...
async def myfeds(client, message):
    is_feds = await get_feds_by_owner(int(message.from_user.id))
    if is_feds:
        response_text = "\n\n".join([f"{i+1}.\n**Fed Name:** {fed['fed_name']}\n**Fed Id:** `{fed['fed_id']}`" for i, fed in enumerate(is_feds)])
...
async def rename_fed(client, message):
    ...
    if len(args) < 3:
        return await message.reply_text("usage: /renamefed <fed_id> <newname>")
    fed_id, newname = args[1], args[2]
    verify_fed = await get_fed_info(fed_id)
    if not verify_fed:
        return await message.reply_text("This fed does not exist in my database!")
    if await is_user_fed_owner(fed_id, message.from_user.id):
        await client.db["federation"].update_one({"fed_id": str(fed_id)}, {"$set": {"fed_name": str(newname)}}, upsert=True)
        await message.reply_text(f"Successfully renamed your fed name to {newname}!")
    else:
...
async def fed_log(client, message):
    if message.chat.type == ChatType.PRIVATE:
        return await message.reply_text("Send this command on the chat which you need to set as fed log channel.")
    member = await app.get_chat_member(message.chat.id, message.from_user.id)
    if member.status not in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]:
        return await message.reply_text("You need to be the chat owner or admin to use this command.")
    if len(message.command) < 2:
        return await message.reply_text("Please provide the Id of the federation with the command!")
    fed_id = message.text.split(" ", 1)[1].strip()
    info = await get_fed_info(fed_id)
    if info is False:
        return await message.reply_text("This federation does not exist.")
    if not (await is_user_fed_owner(fed_id, message.from_user.id)):
        return await message.reply_text("Only federation owner can set logs for this federation.")
    log_group_id = LOG_GROUP_ID if "unsetfedlog" in message.text else message.chat.id
    ...
    return await message.reply_text("log channel set successfully.")
...
async def join_fed(client, message):
    ...
    member = await client.get_chat_member(message.chat.id, message.from_user.id)
    fed_id_existing = await get_fed_id(int(message.chat.id))
    # only group creator (owner) can join; allow global sudo/owner to bypass
    if not (message.from_user.id in (await client.get_chat(message.chat.id)).creator or message.from_user.id in []):
        # simpler check: member.status must be OWNER or the user is a global owner/sudo
        pass
    # Enforce: only chat owner can join normally
    if member.status != ChatMemberStatus.OWNER and message.from_user.id not in (int(OWNER_ID),):
        # allow sudo to skip check
        from config import SUDOERS
        try:
            SUDO = list(map(int, SUDOERS))
        except Exception:
            SUDO = []
        if message.from_user.id not in SUDO and message.from_user.id != int(OWNER_ID):
    ...
    if len(args) > 1:
        ...
        ok = await chat_join_fed(fed_id, message.chat.title, message.chat.id)
        if not ok:
            return await message.reply_text(f"Failed to join federation! Please contact {SUPPORT_CHAT} if this problem persists!")
        if getfed.get("log_group_id"):
            try:
                await app.send_message(getfed["log_group_id"], f'Chat **{message.chat.title}** has joined the federation **{getfed["fed_name"]}**', parse_mode=ParseMode.MARKDOWN)
            except Exception:
                pass
        await message.reply_text(f'This group has joined the federation: {getfed["fed_name"]}!')
    else:
        await message.reply_text("You need to specify which federation you're asking about by giving me a FedID!")
...
async def leave_fed(client, message):
    if message.chat.type == ChatType.PRIVATE:
        return await message.reply_text("This command is specific to groups, not our pm!")
    fed_id = await get_fed_id(int(message.chat.id))
    if not fed_id:
        return await message.reply_text("This chat is not a part of any federation.")
    fed_info = await get_fed_info(fed_id)
    member = await app.get_chat_member(message.chat.id, message.from_user.id)
    if member.status == ChatMemberStatus.OWNER or message.from_user.id in [] or message.from_user.id == int(OWNER_ID):
        ok = await chat_leave_fed(int(message.chat.id))
        if ok:
            if fed_info and fed_info.get("log_group_id"):
                try:
                    await app.send_message(fed_info["log_group_id"], f'Chat **{message.chat.title}** has left the federation **{fed_info["fed_name"]}**', parse_mode=ParseMode.MARKDOWN)
                except Exception:
                    pass
            await message.reply_text(f'This group has left the federation {fed_info["fed_name"]}!')
        else:
            await message.reply_text("How can you leave a federation that you never joined?!")
    else:
...
async def fed_chats(client, message):
    ...
    args = message.text.split(" ", 1)
    if len(args) < 2:
        return await message.reply_text("Please write the Id of the federation!\n\nUsage:\n/fedchats fed_id")
    fed_id = args[1].strip()
    info = await get_fed_info(fed_id)
    if info is False:
        return await message.reply_text("This federation does not exist.")
    fed_owner = int(info["owner_id"])
    fed_admins = [int(x) for x in info.get("fadmins", [])]
    all_admins = [fed_owner] + fed_admins + [int(BOT_ID)]
    if message.from_user.id not in all_admins and message.from_user.id not in [] and message.from_user.id != int(OWNER_ID):
    ...
    await message.reply_text(f"**Here are the list of chats connected to this federation:**\n\n{text}")
...
async def fed_info_cmd(client, message):
    ...
    fed_name = fed_info.get("fed_name", "Unknown")
    owner_mention = fed_info.get("owner_mention", "Unknown")
...
async def get_all_fadmins_mentions(client, message):
    ...
    if not fadmin_ids:
        return await message.reply_text(f"**Owner: {fed_info.get('owner_mention')}\n\nNo fadmins found in the federation.")
    user_mentions = []
    ...
    reply_text = (f"**Owner: {fed_info.get('owner_mention')}\n\nList of fadmins:**\n" + "\n".join(user_mentions))
    await message.reply_text(reply_text)
...
async def fpromote(client, message):
    ...
    fed_id = await get_fed_id(message.chat.id)
    if not fed_id:
        return await message.reply_text("You need to add a federation to this chat first!")
    # permission: only fed owner / sudo / global owner
    if not (await is_user_fed_owner(fed_id, message.from_user.id) or message.from_user.id in [] or message.from_user.id == int(OWNER_ID)):
        return await message.reply_text("Only federation owners can do this!")
    ...
    if check_user:
        ...
        return await message.reply_text(f"**User {user.mention} was Fed Banned.\nYou can unban the user and promote.\n\nReason: {reason}.\nDate: {date}.**")
    getuser = await search_user_in_fed(fed_id, user_id)
    info = await get_fed_info(fed_id)
    get_owner = info["owner_id"]
    if user_id == int(get_owner):
        return await message.reply_text("You do know that the user is the federation owner, right?")
    if getuser:
        return await message.reply_text("I cannot promote users who are already federation admins!")
    if user_id == int(BOT_ID):
        return await message.reply_text("I already am a federation admin in all federations!")
...
async def fdemote(client, message):
    ...
    if user_id is None:
        return await message.reply_text("Failed to extract user from the message.")
    if user_id == int(BOT_ID):
        return await message.reply_text("Cannot demote the bot.")
    if await search_user_in_fed(fed_id, user_id) is False:
        return await message.reply_text("I cannot demote people who are not federation admins!")
...
async def fban_user(client, message):
    ...
    fed_owner = int(info.get("owner_id"))
    all_admins = [fed_owner] + [int(x) for x in fed_admins] + [int(BOT_ID)]
    if message.from_user.id not in all_admins and message.from_user.id not in [] and message.from_user.id != int(OWNER_ID):
        return await message.reply_text("You need to be a Fed Admin to use this command")
    if len(message.command) < 2:
        return await message.reply_text("**You needed to specify a user or reply to their message!**")
    user_id, reason = await extract_user_and_reason(message)
    try:
        user = await app.get_users(user_id)
    except PeerIdInvalid:
        return await message.reply_text("Sorry, I cannot find that user.")
    if not user_id:
        return await message.reply_text("I can't find that user.")
    if int(user_id) in all_admins or int(user_id) == int(OWNER_ID):
    ...
    for served_chat in served_chats:
        try:
            ...
            if chat_member.status == ChatMemberStatus.MEMBER:
                ...
                if served_chat != message.chat.id:
            ...
            pass
    try:
        await app.send_message(user.id, f"Hello, You have been fed banned by {message.from_user.mention}, You can appeal for this ban by contacting them.")
    except Exception:
        pass
...
**Chats:** `{number_of_chats}`"""
    try:
        m2 = await app.send_message(info.get("log_group_id", LOG_GROUP_ID), text=ban_text, disable_web_page_preview=True)
        await m.edit(f"Fed Banned {user.mention} !\nAction Log: {m2.link}", disable_web_page_preview=True)
    except Exception:
        await message.reply_text("User Fbanned, But This Fban Action Wasn't Logged, Add Me In LOG_GROUP")
...
async def funban_user(client, message):
    ...
    check_user = await check_banned_user(fed_id, user_id)
    if not check_user:
        return await message.reply_text("**I can't unban a user who was never fedbanned.**")
    if not reason:
        return await message.reply_text("No reason provided.")
    served_chats, _ = await chat_id_and_names_in_fed(fed_id)
    ...
    for served_chat in served_chats:
        try:
            ...
            pass
    try:
        await app.send_message(user.id, f"Hello, You have been fed unbanned by {message.from_user.mention}, You can thank them for their action.")
    except Exception:
        pass
...
**Chats:** `{number_of_chats}`"""
    try:
        ...
        await message.reply_text("User FUnbanned, But This Fban Action Wasn't Logged, Add Me In LOG_GROUP")
...
async def fedstat(client, message):
    if message.chat.type != ChatType.PRIVATE:
        await message.reply_text("Federation Ban status can only be checked by privately messaging me.")
        return
    if len(message.command) < 2:
    ...
    if not user_id:
        ...
        fed_id = message.text.split(" ", 1)[1].strip()
    if not fed_id:
        return await message.reply_text("Provide me a Fed Id along with the command to search for.")
    info = await get_fed_info(fed_id)
    if not info:
        return await message.reply_text("Please enter a valid fed id")
    check_user = await check_banned_user(fed_id, user_id)
    if check_user:
        user = await app.get_users(user_id)
        reason = check_user["reason"]
...
async def fbroadcast_message(client, message):
    ...
    if not reply_message:
        return await message.reply_text("**You need to reply to a text message to Broadcast it.**")
    ...
    m = await message.reply_text(f"Broadcast in progress, will take {len(chats)*0.1} seconds.")
    for i in chats:
        try:
            await app.send_message(i, text=text, reply_markup=reply_markup)
            sent += 1
...
async def del_fed_button(client, cb):
    query = cb.data
    fed_id = query.split("_", 1)[1]
    if fed_id == "cancel":
        await cb.message.edit_text("Federation deletion cancelled")
        return
    getfed = await get_fed_info(fed_id)
    if not getfed:
        await cb.message.edit_text("Federation not found.")
        return
    # permission: only owner of fed or global owner/sudo can delete
    if not (await is_user_fed_owner(fed_id, cb.from_user.id)):
        await cb.answer("You don't have permission to remove this federation.", show_alert=True)
        return
    # delete
    await client.db["federation"].delete_one({"fed_id": str(fed_id)})
    await cb.message.edit_text(
        f'You have removed your Federation! Now all the Groups that were connected with `{getfed["fed_name"]}` do not have a Federation.',
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def fedtransfer_button(client, cb):
    ...
    if data == "cancel":
        await cb.message.edit_text("Federation transfer cancelled")
        return
    new_owner_id_str, fed_id = data.split("|", 1)
    new_owner_id = int(new_owner_id_str)
    # cb.message.chat.id is the chat id that initiated callback (private)
    initiator_id = cb.from_user.id
    # only current owner can transfer
    ok = await transfer_owner(fed_id, initiator_id, new_owner_id)
    if ok:
        await cb.message.edit_text("**Successfully transferred ownership to new owner.**")
    else:
        await cb.message.edit_text("Transfer failed: you may not be the current owner or something went wrong.")


@app.on_callback_query(filters.regex(r"fed_(.*)"))
async def fed_owner_help(client, cb):
 ...
 • /delfed <fed_id>: Delete a Federation and all associated info.
 • /myfeds: List your federations
 • /fedtransfer <new_owner> <fed_id>: Transfer fed ownership
 • /fpromote <user>: Assigns a federation admin
 • /fdemote <user>: Demote a federation admin
 • /setfedlog <fed_id>: Set a group as fed log channel
 • /unsetfedlog <fed_id>: Remove fed log channel
"""
    elif data == "admin":
        text = """**🔱 Fed Admins:**
 • /fban <user> <reason>: Fed ban a user
 • /sfban: Fban silently (no message)
 • /unfban <user> <reason>: Un-fban a user
 • /sunfban: Unfban silently
 • /fedadmins: Show federation admins
 • /fedchats <FedID>: Get all chats in the federation
 • /fbroadcast: Broadcast a message to all fed chats (reply to text)
"""
    else:
        text = """**User Commands:**
• /fedinfo <FedID>: Information about a federation.
• /fedadmins <FedID>: List the admins in a federation.
• /joinfed <FedID>: Join the current chat to a federation. Chat owners only.
...
"""
    await cb.message.edit(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Back", callback_data="help_module(federation)")]]), parse_mode=ParseMode.MARKDOWN)
